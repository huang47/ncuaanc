...
<?php
    ...
    $thumbnail_dimention = substr($running_line_settings['rnngln_height'], 0, 2);
    if (has_post_thumbnail($post->ID)) {
        $image = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'single-post-thumbnail' );
        echo "<img class=\"ads_image\" src=\"{$image[0]}\" height=\"{$thumbnail_dimention}\" width=\"{$image[1]}\">";
    }
?>
          <?php echo "<p class=\"ads_content\">{$marquee_string}</p>"; ?>
      </marquee>
    </div>
<?php
}
